...
This document is a historical baseline analysis of why Spacebot's pre-working-memory context assembly failed to provide situational awareness, particularly in multi-user, multi-channel environments. It catalogs every failure mode with evidence from live system prompts and conversation transcripts. It is the prerequisite to the working-memory design document.

Status note: the current docs now use layered working memory, participant context, and knowledge synthesis. Sections below intentionally describe the older bulletin-centered baseline.
...
### Baseline behavior before working memory
...
2. **Old compaction path:** In the old design, when context hit >80% capacity, the compactor spawned a worker that summarized old context and wrote extracted durable memories. This only fired when the context window was nearly full.
...
At the old default cadence of 15-minute warmup refreshes: **96 bulletin-era synthesis calls per day**. If each synthesis call uses ~2000 input tokens (raw sections) + ~500 output tokens, that is ~240,000 tokens per day per agent. For an agent that is only active for 4 hours a day, roughly 80% of those generations are wasted on periods of zero activity.
...
**The old compaction memory path compounded the problem.** Active channels hit the compaction threshold faster. The old compaction path extracted memories while summarizing context, but those extracted memories were global (no user_id), the summaries were channel-specific, and the bulletin did not immediately reflect the extracted memories (it waited for the next regeneration cycle).
...
The old compactor path had two jobs: (1) summarize old context to free up space, and (2) extract memories from the context being compacted. Job 1 is fine and should remain. Job 2 is the wrong place for memory creation, for several reasons:
...
**The compaction LLM had the wrong incentive.** Its primary job was to produce a compact summary. Memory extraction was a secondary objective bolted onto the compaction prompt. The LLM optimized for summary quality, not memory coverage.

**Old compaction-extracted memories had no user attribution.** The compaction path stripped sender information from the transcript before processing. Memories extracted during compaction were always global, with no `user_id` or `channel_id` context.
...
| 5 | Token waste on stable info | 80%+ of old bulletin-era synthesis calls are redundant | Medium |
