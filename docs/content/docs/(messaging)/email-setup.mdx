...
folders = ["INBOX"]
allowed_senders = []
sync_max_age_days = 0  # 0 = no limit; only import unread mail newer than N days
```
...
Set `sync_max_age_days` to bound how far back the poller looks. The IMAP server filters server-side, so the check is cheap even on large mailboxes.
...
name = "support"
sync_max_age_days = 7  # import the last week of unread support mail
```
