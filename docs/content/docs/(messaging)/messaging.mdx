...
By default, webhook is inbound/reply-only. Proactive delivery into webhook conversations (for example cross-channel `send_message_to_another_channel` or cron delivery targets like `webhook:my-target`) is disabled unless you set `allow_input = true` in `[messaging.webhook]`.
