...
Native Google Calendar integration that gives workers and channels access to calendar tools — list events, create meetings, find free time, and more. No MCP server needed; credentials are configured directly in Spacebot.

## Overview

When configured, Spacebot registers calendar tools on the appropriate process types:

| Tool | Channels | Workers |
|------|----------|---------|
...
Channels get read-only tools. Workers get both read and write tools. This follows the delegation model — channels can check your schedule, but only workers modify it.
...
```toml
[agents.personal-assistant.google_calendar]
