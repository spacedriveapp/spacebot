...
Spacebot also supports **ACP-backed workers** via [Agent Client Protocol](https://agentclientprotocol.com/). ACP workers run compatible coding agents over stdio and expose them through the same `spawn_worker` tool path.

Configure one or more ACP profiles:
...
Once enabled, the `spawn_worker` tool gains a `worker_type` parameter. The channel LLM decides whether to use `"builtin"` (default), `"opencode"`, or `"acp"` based on the task.
