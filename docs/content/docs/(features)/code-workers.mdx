...
- Keep using **builtin** workers for quick shell/file/exec tasks and non-coding operations.

Both ACP and OpenCode workers are invoked through `spawn_worker`, support interactive follow-ups via `route`, and publish status updates into the channel status block. When neither external backend is selected, Spacebot uses the builtin worker.
