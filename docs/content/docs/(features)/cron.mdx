...
Spacebot separates the concerns. The cortex is the system's heartbeat. It monitors processes, maintains working memory, and regenerates knowledge synthesis when the memory graph changes. Cron jobs are user-defined recurring tasks, each with its own interval, prompt, and delivery target. When a cron job fires, it gets a fresh short-lived channel with normal branching and worker capabilities. Multiple cron jobs run concurrently without blocking each other or the cortex.
...
3. **Run** -- The channel processes the message through its normal LLM loop. It can use channel tools such as `reply`, `branch`, and `spawn_worker`. Memory writes still go through branches or configured direct-mode tools, not the standard channel toolset.
