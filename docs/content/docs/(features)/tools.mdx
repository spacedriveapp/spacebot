...
| `memory_save` | Write a memory to the store | Branch, Cortex |
...
`memory_save`, `memory_recall`, `channel_recall`, `spacebot_docs`, `email_search` on branch ToolServers. `shell`, `file`, `exec` on worker ToolServers. `memory_save` on cortex ToolServers. These are registered before `.run()` via the builder pattern and live for the lifetime of the ToolServer.
...
Compaction workers do not get a ToolServer. They produce a one-turn rolling history summary only; durable memory extraction happens through branches, ingestion, cortex, and silent memory persistence branches.
