...
(The list below is illustrative, not exhaustive; see `create_branch_tool_server()` for the live branch surface.)
...
│   memory_delete    (Arc<MemorySearch>)       │
│   spacebot_docs    (embedded docs)            │
│   channel_recall   (ConversationLogger)      │
│   email_search     (IMAP mailbox search)     │
│   worker_inspect   (run logger)              │
└──────────────────────────────────────────────┘
```

Branch isolation ensures `memory_recall`, `knowledge_recall`, `memory_delete`, `channel_recall`, `spacebot_docs`, `email_search`, and `worker_inspect` are never visible to the channel. All tools are registered at creation and live for the lifetime of the branch.
...
- Native QMD retrieval expects an MCP server configured with the reserved name `qmd`.
