...
`memory_save`, `memory_recall`, `channel_recall`, `spacebot_docs`, and `email_search` on branch ToolServers. `shell`, `file`, and `exec` on worker ToolServers. The cortex ToolServer registers `memory_save`.

The compactor has no tool server. Static tools are registered before `.run()` via the builder pattern and live for the lifetime of the ToolServer.
