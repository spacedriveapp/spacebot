...
The compaction worker runs with a single LLM turn and no tools. It does not write durable memories; memory extraction is handled by branches, ingestion, cortex, and silent memory persistence branches.
...
Durable memories are extracted separately by memory persistence branches and other explicit memory-writing paths. Compaction summaries preserve rolling conversation context but do not write to the memory store.
...
| Memory extraction | Separate pass | Persistence branches and explicit memory tools |
| Raw transcript | Lost | Preserved as rolling compaction summaries |
