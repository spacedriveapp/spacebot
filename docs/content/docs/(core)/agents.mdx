...
- **Identity files** — `SOUL.md`, `IDENTITY.md`, `ROLE.md`, and `SPEECH.md` in the agent root directory (`~/.spacebot/agents/{id}/`)
...
│   │   ├── SPEECH.md                     # spoken cadence, warmth, anti-repetition guidance
...
Identity files are per-agent. `ROLE.md` defines what the agent does — responsibilities, scope, what to handle vs what to escalate. `SPEECH.md` defines how the agent sounds when it speaks — pacing, phrasing, warmth, and repetition constraints for voice output. In single-agent setups this keeps text identity and spoken delivery separate. In multi-agent setups it lets each agent have a distinct voice without changing its operational role.
