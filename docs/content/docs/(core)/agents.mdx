...
Boss receives user request
    → send_agent_message → Planning-lead task store
        ...
        → Injects identity (SOUL.md, IDENTITY.md, ROLE.md) + org_context
...
6. **Synthesis and report** — The engineering assistant reads results with `task_get`, synthesizes findings, updates the task, and reports back. The planning-lead synthesizes and reports to the boss.
