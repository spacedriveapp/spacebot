...
The communication graph enables a structured delegation pattern: the **boss hierarchy**. This is a multi-tier org chart where a boss agent delegates to a planning-lead, which orchestrates builder workers to execute tasks.
...
The `hierarchical` link kind establishes that `boss` manages `planning-lead`. The boss can delegate tasks down, and the planning-lead can escalate issues up.

### How It Works

**Delegation flow:**

1. **Boss delegates** — The boss agent uses `send_agent_message` to create a task in the planning-lead's task store. This appears as a message in the planning-lead's link channel (`link:planning-lead:boss`).
2. **Planning-lead orchestrates** — The planning-lead's cortex picks up `Ready` tasks from its task board and spawns builder workers. Each builder gets shell, file, and browser tools plus a focused system prompt.
3. **Builders execute** — Workers run autonomously. They can create subtasks via `task_create` for escalation when they hit blockers.
4. **Completion flows back** — When builders finish, results propagate up through the planning-lead to the boss, and ultimately back to the originating user channel.
