---
title: Memory
description: Graph memory, working memory, and knowledge synthesis for durable recall plus day-of awareness.
---

# Memory

Spacebot has two complementary memory layers:

- **Graph memory** for durable knowledge, semantic recall, and long-lived relationships
- **Working memory** for recent operational awareness: what happened today, what other channels are doing, and what decisions were just made

Both are rendered into channel prompts differently. Graph memory is recalled and synthesized. Working memory is rendered directly from the event log plus compact daily/intra-day summaries. Together they replace the old flat `MEMORY.md` / bulletin style of agent context.
...
- **SQLite** -- the memory graph **and** working-memory event log. Graph rows with content, type, importance, timestamps, source. Association edges with weights and relation types. Working-memory events, synthesis blocks, and channel activity rendering also live here.
...
Graph-memory recall joins the two on memory ID. A recall worker queries LanceDB for semantic/keyword matches, then hits SQLite for graph traversal and metadata. Working memory does not use embeddings at all — it is SQL-first, temporal, and renderable even before an embedding model is warm.
...
Working memory is not a second vector store. It is an append-only operational log plus synthesized summaries that answer questions like:

- What happened earlier today?
- What did we decide a few channels ago?
- Which workers or branches just finished?
- What is this channel focused on right now?

Typical working-memory events include worker spawns and completions, branch completions, task updates, system events, cron executions, errors, and high-value decisions surfaced by the persistence path.

### Why It Exists

Graph memory is optimized for durable knowledge and semantic retrieval. It is great for "what do we know about auth?" It is not ideal for "what happened in the last two hours across active channels?" Working memory fills that gap.

Channels render working memory directly into prompt context, so a freshly resumed conversation can still see today's narrative and channel activity without first spawning a branch.

### What Channels See

Channel prompts now include:

1. Identity files (`SOUL.md`, `IDENTITY.md`, `ROLE.md`)
2. Knowledge synthesis from the cortex
3. Rendered working-memory timeline for the current day
4. A compact channel activity map showing what else is active

This gives the agent both durable knowledge and temporal awareness.

### Working Memory vs Graph Memory

| Layer | Best for | Backing store | Retrieval style |
|------|----------|---------------|-----------------|
| Graph memory | facts, preferences, observations, long-term decisions | SQLite + LanceDB | branch recall + synthesis |
| Working memory | recent decisions, completions, errors, current activity | SQLite | direct prompt rendering + synthesis |
| Knowledge synthesis | high-level cross-cutting agent knowledge | runtime cache backed by cortex generation | direct prompt injection |
...
- **SPEECH.md** -- spoken cadence, delivery style, warmth, and anti-repetition rules for voice output

These are loaded into channel system prompts every time. `SPEECH.md` is also used during spoken-reply generation when Voicebox output is enabled. They are files in the agent root directory (not the workspace), not database rows, because they change rarely and users should be able to edit them in a text editor or through the config UI.
...
When a channel starts or a cron job fires, memory context is layered rather than dumped as one blob:

1. **Identity files** -- stable agent persona and role
2. **Knowledge synthesis** -- the cortex's compact high-level understanding of the agent's world
3. **Working memory** -- recent event narrative plus raw tail for today's activity
4. **Channel activity map** -- other active conversations and their current focus
5. **On-demand recall** -- branches pull graph memories when the conversation needs deeper specific context

Not a wall of raw search results. Not everything in the database. Just the right mix of durable knowledge and recent situational awareness.
