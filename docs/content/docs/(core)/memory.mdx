...
### 2. Persistence-branch initiated (background extraction)

Silent memory persistence branches periodically review conversation history, recall existing memories to avoid duplicates, and save durable facts, preferences, decisions, and observations. They run in the background and complete silently, so the channel stays focused on responding to the user.

Persistence branches are the main safety net for conversations that contain important context but never hit the compaction threshold. They can also be triggered earlier by working-memory signals such as decisions, worker completions, cron executions, and other high-signal events.

### 3. Ingestion-initiated (document extraction)

Ingestion workers process uploaded or watched documents in chunks and save extracted memories through the same memory tools as branches. This path is for turning source material into durable knowledge without relying on a conversation turn.
...
Compaction is not a memory creation path. The compactor summarizes old conversation turns so the channel can keep a rolling history, but it has no memory tools and does not write durable memories.
...
This costs an extra LLM call. But it keeps the channel's context clean, which means fewer compactions and less pressure on context-management machinery overall.
