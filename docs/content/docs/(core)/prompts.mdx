...
`memory_bulletin` remains a legacy fallback. When `knowledge_synthesis` is available, the channel prompt renders `## Knowledge Context`; `## Memory Context` only renders when knowledge synthesis is absent.
