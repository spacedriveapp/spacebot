...
│  Maintains knowledge synthesis and working memory       │
│  context injected into every channel's prompt.          │
└─────────────────────────────────────────────────────────┘
```
...
| `SpokenResponse` | Channel | UI | Short spoken rewrite for TTS playback |
...
| `working_memory_events` / synthesis tables | Temporal event log and compressed daily / intra-day summaries |
...
| `cortex_events` | Cortex action log (knowledge synthesis, maintenance, supervision) |
...
- **settings.redb** — runtime settings (worker_log_mode, spoken-response history, other per-channel runtime state)
...
5. Responses are also forwarded to SSE clients (WebChat, dashboard, desktop voice overlay) for real-time UI updates

### Voice Interaction Path

The voice UI is layered on top of the same messaging and SSE pipeline rather than being a separate agent mode.

1. The overlay captures microphone audio and uploads it through the WebChat audio API
2. The channel handles the resulting user turn normally
3. After a reply is sent, the channel optionally generates a shorter spoken rewrite when `voicebox_url` is configured
4. `SPEECH.md` guidance and the last 15 spoken rewrites are injected into that speech-generation prompt to reduce repetition
5. A `SpokenResponse` SSE event is emitted with a stable message ID so the UI can pair the spoken rewrite with the exact outbound assistant message
6. The overlay or WebChat UI sends the spoken text to Voicebox for playback while rendering live FFT-driven branded waves
...
| Cortex | 10 | Knowledge synthesis, maintenance, and system observation. |
...
The API supports Server-Sent Events (SSE) for real-time streaming to connected clients. Status updates, tool call progress, worker lifecycle events, spoken-response events, and memory changes are all pushed via SSE, giving the dashboard, WebChat, and voice overlay live visibility into agent activity.
...
CLI (clap) → parse args
  ...
  → run()
    ...
    → Start Cortex loops (warmup, knowledge synthesis, working memory maintenance, association, ready-task)
...
**Prompts are files.** System prompts live in `prompts/` as Jinja2 templates, not as string constants in Rust code. Identity files (`SOUL.md`, `IDENTITY.md`, `ROLE.md`, `SPEECH.md`) live in the agent root directory — outside the workspace sandbox — so worker file tools cannot access them.
