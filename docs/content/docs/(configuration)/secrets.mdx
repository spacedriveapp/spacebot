...
For a provider's `api_key`, the resolution chain is:

```
[llm.provider.<id>].api_key (secret: / env: / literal)
  → missing (config load fails for that provider)
```

If `api_key` is `"secret:ANTHROPIC_API_KEY"` and the secret store has that key,
it resolves to the stored value.

Separately, `ANTHROPIC_API_KEY` (or `ANTHROPIC_AUTH_TOKEN`) and
`LITELLM_API_KEY` bootstrap an `anthropic` / `litellm` provider when the config
file does not define one under that id. No other `*_API_KEY` variable
...
Provider keys (`[llm.provider.*].api_key`) are **not** scanned. They already accept `secret:` / `env:` references directly, so there is no flat `llm.*_key` field left to migrate -- write the reference yourself.
