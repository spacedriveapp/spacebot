...
# --- MCP Servers (optional, requires --features mcp) ---
# Instance-level server definitions. Agents reference these by ID.
[[mcp_servers]]
id = "github"
transport = "stdio"
command = "npx"
...
Requires `--features mcp`. See [MCP Tools](/docs/mcp) for full details.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `servers` | string[] | `[]` | MCP server IDs to connect to (references `[[mcp_servers]]` entries) |

### `[[mcp_servers]]`

Instance-level MCP server definitions. Requires `--features mcp` at build time.

| Key | Type | Required | Description |
|-----|------|----------|-------------|
| `id` | string | Yes | Unique server identifier |
| `transport` | string | Yes | `"stdio"` (child process) |
...
Agent-specific MCP servers are set via `[agents.mcp]` with the same keys as `[defaults.mcp]`. The per-agent list replaces (not merges with) the defaults.
