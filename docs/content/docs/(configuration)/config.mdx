...
# Working memory (temporal context rendered into channel prompts).
[defaults.working_memory]
enabled = true
...
# Optional voice output integration for spoken replies.
[agents.voice]
voicebox_url = "http://localhost:9990"
...
| Identity files (SOUL.md, etc.) | Yes | Next channel message renders new identity and speech style |
...
- Each agent's root directory (identity files: SOUL.md, IDENTITY.md, ROLE.md, SPEECH.md)
...
└── agents/
    └── main/
        ...
        ├── SPEECH.md              # spoken reply style (hot-reloaded)
...
Controls the temporal context layer injected into channel prompts.

| Field | Type | Default | Description |
|------|------|---------|-------------|
| `enabled` | bool | `true` | Enables working-memory rendering in channel prompts. |
| `intraday_batch_threshold` | int | `15` | Number of unsynthesized events before an intra-day synthesis pass is triggered. |
| `intraday_time_fallback_secs` | int | `14400` | Time-based fallback for synthesis when event volume is low. |
| `today_max_unsynthesized_events` | int | `10` | Raw event tail preserved alongside synthesized narrative. |
| `context_token_budget` | int | `1500` | Token budget for the full working-memory section. |
| `channel_map_token_budget` | int | `300` | Token budget for the cross-channel activity map. |
| `channel_map_max_channels` | int | `10` | Maximum channels included in the activity map. |
| `channel_map_inactive_hours` | int | `24` | Hide inactive channels after this many hours. |
| `min_importance_under_pressure` | float | `0.5` | Importance cutoff when the section is under token pressure. |
| `event_retention_days` | int | `30` | Retention window for raw working-memory events. |
| `daily_summary_max_words` | int | `300` | Word budget for daily rollup summaries. |
| `persistence_message_threshold` | int | `20` | Trigger memory persistence after this many messages. |
| `persistence_time_threshold_secs` | int | `900` | Time-based persistence trigger. |
| `persistence_event_density_threshold` | int | `5` | Trigger persistence when enough high-signal events accumulate. |

### `agents.voice`

Voice output is optional and configured per agent.

| Field | Type | Default | Description |
|------|------|---------|-------------|
| `voicebox_url` | string | empty | Base URL for the Voicebox server used for TTS profile lookup and spoken reply generation. When unset, text chat still works and no spoken-response events are emitted. |

When voice output is enabled, Spacebot generates a shorter spoken rewrite for outbound assistant messages, applies `SPEECH.md` guidance, injects the last 15 spoken rewrites into the speech-generation prompt to reduce repetition, and emits `spoken_response` SSE events for the web/desktop voice UI.
