...
LLM keys also have implicit env fallbacks — if no key is set in the TOML, Spacebot checks `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `OPENROUTER_API_KEY`, `KILO_API_KEY`, `REQUESTY_API_KEY`, and `OPENCODE_GO_API_KEY` automatically.
...
| Requesty | `requesty/<provider>/<model>` | `requesty/openai/gpt-4o-mini` |
...
| `requesty_key` | string | None | Requesty API key (`secret:NAME`, `env:VAR_NAME`, or literal) |
