...
| `worker_contract` | table | inherits | Per-agent worker contract override (`AgentConfig.worker_contract`) |

Agent-specific routing is set via `[agents.routing]` with the same keys as `[defaults.routing]`.

### `[agents.worker_contract]`

Per-agent worker contract override for `AgentConfig.worker_contract`.
Unset keys inherit from `[defaults.worker_contract]`.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `ack_secs` | integer | inherits | Seconds before first worker acknowledgement checkpoint |
| `progress_secs` | integer | inherits | Seconds between worker progress deadline checks |
| `tick_secs` | integer | inherits | Scheduler tick interval for contract scans |
