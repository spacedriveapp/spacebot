...
- `~/.spacebot/agents/main/` — identity files (`SOUL.md`, `IDENTITY.md`, `ROLE.md`, `SPEECH.md`)
...
| `SPEECH.md`   | Spoken cadence, delivery style, anti-repetition rules |

Template files are created on first run. Edit them to shape the agent's personality and speaking style. Changes are hot-reloaded (no restart needed). These files live outside the workspace so they are not accessible to worker file tools.
