...
export function ConnectionScreen() {
  ...
  const handleStartLocal = useCallback(async () => {
    ...
    try {
      ...
      const spawned = await spawnBundledProcess(
        "binaries/spacebot",
        ["start", "--foreground"],
        {
          onError: (error) => {
          ...
          onClose: (data) => {
            if (!sawReady || data.code === null || data.code !== 0) {
              ...
              setSidecarError(
                data.code === null
                  ? `Process exited before the HTTP server became ready${stderr}`
                  : `Process exited with code ${data.code}${stderr}`,
              );
              return;
            }
      ...
      <div className="flex w-full max-w-md flex-col items-center gap-8 px-6">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-center text-sm text-ink-dull">
            Enter the URL of a running Spacebot instance, or start one locally.
          </p>
        </div>
        ...
        <div className="flex w-full flex-col gap-3">
          ...
          {isChecking ? (
            ...
            <p className="text-xs text-ink-faint">Not connected</p>
          ) : null}
        </div>
            ...
            <div className="flex w-full flex-col gap-3">
              ...
              {sidecarState === "starting" && (
                <p className="text-xs text-ink-faint">
                  Starting the bundled Spacebot binary. This may take a few
        ...
        <p className="text-center text-xs text-ink-faint">
          ...
          {!hasBundledServer && (
            <>
              {" "}
              Install via{" "}
              <span className="font-mono text-ink-dull">docker</span> or
              download from{" "}
              <a
