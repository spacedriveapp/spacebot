...
pub(super) async fn get_providers(
    ...
    let (anthropic, openai, openrouter, zhipu, groq, together, fireworks, deepseek, xai, mistral, opencode_zen, featherless) = if config_path.exists() {
        ...
        (
            ...
            has_key("featherless_key", "FEATHERLESS_API_KEY"),
        )
    } else {
        (
...
pub(super) async fn update_provider(
    ...
    let should_set_routing = {
        ...
        let has_key_for_current = match current_provider {
            ...
            "featherless" => has_provider_key("featherless_key", "FEATHERLESS_API_KEY"),
            _ => false,
        };
