...
pub struct RuntimeConfig {
    ...
    /// Warm memory set used as a fast-path for branch recall.
    pub warm_recall_memories: ArcSwap<Vec<crate::memory::types::Memory>>,
    /// Last refresh timestamp for the warm recall memory set.
    pub warm_recall_refreshed_at_unix_ms: ArcSwap<Option<i64>>,
    /// Synchronizes warm recall cache writers (warmup refresh and memory eviction).
    pub warm_recall_cache_lock: Arc<tokio::sync::Mutex<()>>,
    /// Memory IDs currently being forgotten with per-ID in-flight reference
    /// counts; warm fallback excludes any ID with a count > 0.
    pub warm_recall_inflight_forget_counts:
        Arc<std::sync::Mutex<std::collections::BTreeMap<String, usize>>>,
    /// Monotonic epoch for warm recall cache mutations.
    pub warm_recall_cache_epoch: std::sync::atomic::AtomicU64,
...
impl RuntimeConfig {
    ...
    pub fn new(
        ...
        Self {
            ...
            warm_recall_memories: ArcSwap::from_pointee(Vec::new()),
            warm_recall_refreshed_at_unix_ms: ArcSwap::from_pointee(None),
            warm_recall_cache_lock: Arc::new(tokio::sync::Mutex::new(())),
            warm_recall_inflight_forget_counts: Arc::new(std::sync::Mutex::new(
                std::collections::BTreeMap::new(),
            )),
            warm_recall_cache_epoch: std::sync::atomic::AtomicU64::new(0),
