...
impl Config {
    ...
    fn from_toml(toml: TomlConfig, instance_dir: PathBuf) -> Result<Self> {
        ...
        // Apply env var overrides to defaults. These override the config file but do not
        // override per-agent settings (per-agent configs inherit from defaults at merge time).
        if let Ok(url) = std::env::var("SPACEBOT_BROWSER_CONNECT_URL") {
            defaults.browser.connect_url = Some(url);
        }
